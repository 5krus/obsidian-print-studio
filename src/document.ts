...
export function cleanMarkup(html: string): string {
  const clean = DOMPurify.sanitize(html, {USE_PROFILES:{html:true,svg:true,svgFilters:true}, ADD_ATTR:['xmlns'], FORBID_TAGS:['style','input','button','iframe','object','embed','video','audio','form'], FORBID_ATTR:['style','srcset']});
  const root = document.createElement('div'); root.innerHTML=clean;
  root.querySelectorAll('img').forEach(img=>{if(!/^data:image\/(png|jpeg|gif|webp|svg\+xml);(?:base64,|charset=utf-8,)/i.test(img.src)) {const alt=document.createElement('span');alt.className='ps-image-placeholder';alt.textContent=`[Image: ${img.alt || 'unavailable'}]`;img.replaceWith(alt);}});
...
export function frameDocument(job: PrintJob, token: string): string {
  const payload=JSON.stringify({...job,preset:normalizePreset(job.preset),html:cleanMarkup(job.html),token}).replace(/</g,'\\u003c');
  ...
  html,body{background:#e8ebe8;margin:0}.pagedjs_pages{display:flex;flex-direction:column;align-items:center;gap:24px;padding:24px}.pagedjs_page{background:white;box-shadow:0 4px 20px #182a2017;flex-shrink:0}.pagedjs_pagebox{position:relative}#ps-loading{font:14px Arial;padding:40px;text-align:center;color:#4c6259}
