...
export function cleanMarkup(html: string): string {
  ...
  root.append(clean);
  root.querySelectorAll('input').forEach(input=>{if(input.type==='checkbox'){input.closest('li')?.classList.add('task-list-item');const mark=document.createElement('span');mark.className='ps-check';mark.textContent=input.checked?'☑\uFE0E':'☐';input.replaceWith(mark);}else input.remove();});
