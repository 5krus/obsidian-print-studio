...
export type FrameMessage =
  ...
  | {type:'output'; token:string; kind:'pdf'|'print'; html:string};
export type FrameCommand =
  ...
  | {type:'print'|'pdf'|'export'; token:string};
...
export function frameMessage(data:unknown):FrameMessage|undefined {
  ...
  if(value.type==='output' && (value.kind==='pdf' || value.kind==='print') && typeof value.html==='string')return {type:'output',token,kind:value.kind,html:value.html};
...
export function frameCommand(data:unknown):FrameCommand|undefined {
  const value=record(data);if(!value || typeof value.token!=='string')return;
  const token=value.token;
  if(value.type==='print' || value.type==='pdf' || value.type==='export')return {type:value.type,token};
